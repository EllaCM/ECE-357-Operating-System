...
int main(int argc, char ** argv ){
	...
	char * mapped_area = mmap(NULL,(size_t)fileSize, PROT_WRITE, MAP_PRIVATE, fd, 0);
	if(mapped_area==MAP_FAILED){
		 fprintf(stderr,"Failed to mmap testfile[%s]: %s\n",FILENAME,strerror(errno));
		 exit(EXIT_FAILURE);
	}
	char Strtowrite[] = "This Assignment is real fun. dont let b bother us";
	...
	if (!strcmp(buff,Strtowrite)) {
		flag=0;
		fprintf(stderr,"Update to an mmapped file with MAP_PRIVATE is visible to read(2).\n");
	} else {
		flag=1;
		fprintf(stderr,"Update to an mmapped file with MAP_PRIVATE is not visible to read(2).\n");
	}
	if(close(fd)==-1){
