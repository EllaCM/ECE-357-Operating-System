...
int bytecomp(char *path1, char *path2){
	...
	if ((fp1 != NULL) && (fp2 != NULL)){
		...
		if(fclose(fp1)==EOF){
			printf("Warning:Error in closing file %s:%s\n",path1,strerror(errno));
		}
		if(fclose(fp2)==EOF){
			printf("Warning:Error in closing file %s:%s\n", path2,strerror(errno));
		}
...
char* READSYMLINK(char* currentpath,char* dname, const struct stat* STAT);
...
int processstat(char* dname,const struct stat target,char* targetpath,unsigned int walk_permission,char* searchpath,unsigned int call_source){
   ...
   if (lstat(path, &sb) == -1) {
	   ...
	   return(EXIT_FAILURE);}    /*leave this path or item*/
   /*printf ("%04x/%lld\t", sb.st_dev, sb.st_ino);printf("Mode:%lo (octal)\n",(unsigned long) sb.st_mode);*/
   switch (sb.st_mode & S_IFMT) {
    case S_IFDIR:{
    		hunt(target,targetpath,path,call_source);
    		break;}
    case S_IFLNK:{
    		unsigned int link_permission=0;
    		...
    		if (lstat(path, &symlink) == -1) {
    			   ...
    			   break;}
    		/*if(symlink.st_mode&S_IXOTH) {link_permission=1;}     //follow symlink solution with permission check attempt
    		* processstat(resolvedlink,target,targetpath,link_permission&&walk_permission,searchpath,soft_link); //need inode check argument to avoid loop
    		*/
    		unsigned int soft_link=1;
    		if(symlink.st_mode&S_IFREG){ /*proceed if type is a file*/
    			processstat(resolvedlink,target,targetpath,walk_permission,searchpath,soft_link); /*walk_permission not accureat, irrelevent either*/
    		}
    		break;}
    case S_IFREG: /*regular file ::lets comp\n*/
    		...
    		if(currentfile_size==target.st_size){
    			if(!bytecomp(path,targetpath)){
    				...
    				if(!call_source){     /*condition removed for symlink follow solution attempt*/
    		...
    		break;}
    default:       printf("Warning: (%s) under path [%s] links to something not a file/dir/symlink,skipping",dname,searchpath);   break;
    }
   return 0;
}
static int filter (const struct dirent *unfiltered);
int hunt(const struct stat target,char* targetpath,const char* searchpath,unsigned int call_source){
	/*get cwd's permmision for real path: assuming other user have permission to call this program and is nder current directory*/
	unsigned int walk_permission=0;
	/*char *realPath = realpath (searchpath, NULL);    // use if assumption changed to non-user/non-group call from root/dir
     if (realPath == NULL){
		printf(stderr,"Warning:realpath() failure for searchpath(%s): %s",searchpath,strerror (errno));
	}*/
	struct stat walk;
	/*fputs("HUNT current real path",stdout); puts(realPath);*/
	...
	if (n >= 0){
		for (int cnt = 0; cnt < n; ++cnt){
		  /*fprintf(stdout,"printing element number:%i\t",cnt); puts(concat("file:",concat("./",eps[cnt]->d_name)));*/
...
int main (int argc, char *argv[]){
  ...
  /*program assume the target is relative to cwd*/
	...
	/*printf ("target info:%04x/%lld\n", target.st_dev, target.st_ino);Debug info*/
	struct stat search;
